...
int main(int argc, char* argv[])
{
  ...
  int thickness =3;//how thick do you want the x to be?
  int thickTemp = thickness -1;
  bool displayPic = false;
 
  if (argc == 1)
    {
      cout << "USAGE: ./m152a nameOfArray (outputs an array for X)\n./m152a picture (outputs a picture corresponding to the array for X)";
	exit(1);//arrName = "arr";
    }
  ...
  for(int i = 0; i < 50; i++)
    for(int j = 0; j < 50; j++)
      if ((i >= j-thickTemp && i<= j || (i >=(50-j-thickTemp) && i <= 50-j))
...
void displayPicture(int i, int j)
{  if(arr[i][j] == 1) cout << 1;  else cout << " ";

}


void displayArray(int i, int j)
{  if(arr[i][j] == 1) cout <<arrName <<  "[" << i << "][" << j << "]="<<arr[i][j]<< ";  " ;
